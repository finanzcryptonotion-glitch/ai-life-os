...
export async function POST() {
  ...
  try {
    ...
    for (const [date, d] of Object.entries(byDate)) {
      ...
      await run(`DELETE FROM income_entries WHERE date = ? AND category = 'E-Commerce'`, [date]);
      await run(`INSERT INTO income_entries (amount, category, description, date) VALUES (?, 'E-Commerce', ?, ?)`,
        [d.revenue, `Shopify Orders (${d.orders}x)`, date]);

      await run(`DELETE FROM ecommerce_entries WHERE date = ?`, [date]);
      await run(`INSERT INTO ecommerce_entries (revenue, profit, ad_spend, orders, conversion_rate, notes, date) VALUES (?, ?, 0, ?, 0, ?, ?)`,
        [d.revenue, net, d.orders, notes, date]);
      synced++;
    }
